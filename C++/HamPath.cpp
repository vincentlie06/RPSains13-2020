...
void dfs(int node,vector<vector<vector<int>>> updat-ed_con,vector<int>& storage,int node_count,vector<vector<int>>& ham_paths, vector<int>& visited){
    int updated_con_size=(updated_con[node]).size();
    for(int a=0; a<updated_con_size;a+=1){
        int current_node=updated_con[node][a][1];
        if(!visited[current_node]){
            storage.push_back(current_node); visited[current_node] = 1;
            dfs(current_node,updated_con,storage,node_count,ham_paths);
            storage.pop_back(); visited[current_node] = 0;
        }
    }
...
void find_ham(int node,vector<vector<vector<int>>> updated_con,int node_count,vector<vector<int>>& ham_paths, vector<int>& visited){
    vector<int> storage;
    storage.push_back(node); visited[node] = 1;
    dfs(node,updated_con,storage,node_count,ham_paths, visited);
    visited[node] = 0;
}
...
int main(){
    ...
    for(int x=0; x<node_count; x+=1){
        find_ham(x,updated_con,node_count,ham_paths,visited);
    }
